...
use crate::services::{
    process_bgptool_query_blocking,
    process_email_search_blocking,
    process_geo_query_blocking,
    process_rir_geo_query_blocking,
    process_prefixes_query_blocking,
    process_irr_query_blocking,
    process_looking_glass_query_blocking,
    process_manrs_query_blocking,
    process_rpki_query_blocking,
    blocking_query_whois,
    blocking_query_with_iana_referral,
};
            ...
            Ok(mut stream) => {
                ...
                let response = match &query_type {
                    ...
                    QueryType::Dns(base_query) => {
                        ...
                        Ok(
                            format!("DNS queries are only supported on the async server.\nPlease use the main server (port 43) for DNS lookups.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Trace(base_query) => {
                        ...
                        Ok(
                            format!("Traceroute queries are only supported on the async server.\nPlease use the main server (port 43) for traceroute.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Ssl(base_query) => {
                        ...
                        Ok(
                            format!("SSL certificate queries are only supported on the async server.\nPlease use the main server (port 43) for SSL queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Crt(base_query) => {
                        ...
                        Ok(
                            format!("Certificate Transparency queries are only supported on the async server.\nPlease use the main server (port 43) for CRT queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Minecraft(base_query) => {
                        ...
                        Ok(
                            format!("Minecraft server queries are only supported on the async server.\nPlease use the main server (port 43) for Minecraft queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::MinecraftUser(base_query) => {
                        ...
                        Ok(
                            format!("Minecraft user queries are only supported on the async server.\nPlease use the main server (port 43) for Minecraft user queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Steam(base_query) => {
                        ...
                        Ok(
                            format!("Steam game/user queries are only supported on the async server.\nPlease use the main server (port 43) for Steam queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::SteamSearch(base_query) => {
                        ...
                        Ok(
                            format!("Steam game search queries are only supported on the async server.\nPlease use the main server (port 43) for Steam search queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Imdb(base_query) => {
                        ...
                        Ok(
                            format!("IMDb movie/TV show queries are only supported on the async server.\nPlease use the main server (port 43) for IMDb queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::ImdbSearch(base_query) => {
                        ...
                        Ok(
                            format!("IMDb search queries are only supported on the async server.\nPlease use the main server (port 43) for IMDb search queries.\nQuery: {}\n", base_query)
                        )
                    }
                    ...
                    QueryType::Aosc(base_query) => {
                        ...
                        Ok(
                            format!("AOSC package queries are only supported on the async server.\nPlease use the main server (port 43) for AOSC queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Aur(base_query) => {
                        ...
                        Ok(
                            format!("AUR package queries are only supported on the async server.\nPlease use the main server (port 43) for AUR queries.\nQuery: {}\n", base_query)
                        )
                    }
                    ...
                    QueryType::Ubuntu(base_query) => {
                        ...
                        Ok(
                            format!("Ubuntu package queries are only supported on the async server.\nPlease use the main server (port 43) for Ubuntu queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::NixOs(base_query) => {
                        ...
                        Ok(
                            format!("NixOS package queries are only supported on the async server.\nPlease use the main server (port 43) for NixOS queries.\nQuery: {}\n", base_query)
                        )
                    }
                    ...
                    QueryType::Npm(base_query) => {
                        ...
                        Ok(
                            format!("NPM package queries are only supported on the async server.\nPlease use the main server (port 43) for NPM queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Pypi(base_query) => {
                        ...
                        Ok(
                            format!("PyPI package queries are only supported on the async server.\nPlease use the main server (port 43) for PyPI queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::GitHub(base_query) => {
                        ...
                        Ok(
                            format!("GitHub user/repository queries are only supported on the async server.\nPlease use the main server (port 43) for GitHub queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Wikipedia(base_query) => {
                        ...
                        Ok(
                            format!("Wikipedia article queries are only supported on the async server.\nPlease use the main server (port 43) for Wikipedia queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Lyric(base_query) => {
                        ...
                        Ok(
                            format!("Luotianyi lyric queries are only supported on the async server.\nPlease use the main server (port 43) for lyric queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Cargo(base_query) => {
                        ...
                        Ok(
                            format!("Cargo package queries are only supported on the async server.\nPlease use the main server (port 43) for Cargo queries.\nQuery: {}\n", base_query)
                        )
                    }
                    QueryType::Meal => {
                    ...
                    QueryType::Unknown(q) => {
                        ...
                        if
                            q.to_uppercase().ends_with("-DN42") ||
                            q.to_uppercase().ends_with("-MNT")
                        {
