...
const NEXTTRACE_LINUX_URL: &str = "https://github.com/nxtrace/NTrace-core/releases/download/v1.4.0/nexttrace_linux_amd64";
...
impl NextTraceManager {
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    ...
                    debug!("nexttrace will run without special capabilities - some features may be limited");
                }
            }
            Err(e) => {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        let response = client.get(download_url).send().await.map_err(|e| {
            anyhow::anyhow!("Failed to download NextTrace binary: {}", e)
        })?;
        ...
        let binary_data = response.bytes().await.map_err(|e| {
            anyhow::anyhow!("Failed to read NextTrace binary data: {}", e)
        })?;

        // Write binary to file
        fs::write(&self.binary_path, binary_data).await.map_err(|e| {
            anyhow::anyhow!("Failed to write NextTrace binary: {}", e)
        })?;
    ...
    pub async fn trace_route(&self, target_ip: &str) -> Result<String> {
        ...
        let output = cmd.output().await.map_err(|e| {
            anyhow::anyhow!("Failed to execute NextTrace: {}", e)
        })?;
        ...
        Ok(manager_arc) => {
            ...
            if !manager.initialized {
                if let Err(e) = manager.initialize().await {
                    ...
                    return Ok(format!("Traceroute service initialization failed: {}\n\nPlease ensure internet connectivity for initial setup.\n", e));
                }
            }
