...
fn format_pypi_response(package: &PyPIResponse, query: &str) -> String {
    ...
    if let Some(requires_dist) = &info.requires_dist {
        if !requires_dist.is_empty() {
            let deps: Vec<String> = requires_dist.iter()
                .take(10)
                .map(|dep| {
    ...
    if let Some(classifiers) = &info.classifiers {
        let lang_classifiers: Vec<&String> = classifiers.iter()
            .filter(|c| c.starts_with("Programming Language"))
            .take(3)
            .collect();
        if !lang_classifiers.is_empty() {
            let langs: Vec<String> = lang_classifiers.iter()
        ...
        let status_classifiers: Vec<&String> = classifiers.iter()
    ...
    if let Some(urls) = &package.urls {
        ...
        let wheel_count = urls.iter().filter(|u| u.packagetype == "bdist_wheel").count();
    ...
    output.push_str(&format!("pypi-url: https://pypi.org/project/{}/\n", urlencoding::encode(&info.name)));
    output.push_str(&format!("api-url: {}{}/json\n", PYPI_API_URL, urlencoding::encode(&info.name)));
...
fn format_pypi_not_found(package_name: &str) -> String {
    format!(
        ...
        package_name, urlencoding::encode(package_name)
    )
}
