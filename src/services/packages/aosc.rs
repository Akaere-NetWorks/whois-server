...
fn parse_aosc_html(html: &str, query: &str) -> Result<AOSCSearchResponse> {
    ...
    let version = version_regex.captures(html)
    ...
    let section = section_regex.captures(html)
        .and_then(|cap| cap.get(1))
        .map(|m| m.as_str().trim().to_string());
    
    // Extract dependencies - only runtime dependencies, not build or library
    let depends_regex = Regex::new(r#"<b class="pkg-field pkg-dep-rel">Depends</b>\s*:\s*\n\s*((?:<span class="pkg-dep"><a href="[^"]+">([^<]+)</a></span>,?\s*)+)"#).unwrap();
    ...
    if let Some(cap) = depends_regex.captures(html) {
        ...
        for dep_cap in dep_name_regex.captures_iter(deps_html) {
            if let Some(dep_name) = dep_cap.get(1) {
                ...
                if !dep_str.starts_with("http") && !dep_str.starts_with("/") && !dep_str.contains("github.com") {
    ...
    let upstream_url = {
        ...
        let upstream_regex1 = Regex::new(r#"<b class="pkg-field"[^>]*>Upstream</b>:\s*<a href="([^"]+)">source</a>"#).unwrap();
        let url1 = upstream_regex1.captures(html)
            .and_then(|cap| cap.get(1))
        ...
        if url1.is_some() {
            ...
            let upstream_regex2 = Regex::new(r#"<a href="([^"]+)"\s*>\(tarball\)[^<]*</a>"#).unwrap();
            upstream_regex2.captures(html)
                .and_then(|cap| cap.get(1))
                .map(|m| m.as_str().to_string())
    ...
    let upstream_version = upstream_ver_regex.captures(html)
        .and_then(|cap| cap.get(1))
        .map(|m| m.as_str().trim().to_string());
    ...
    for cap in arch_size_regex.captures_iter(html) {
        if let (Some(arch_name), Some(size)) = (cap.get(1), cap.get(2)) {
            ...
            if matches!(arch, "amd64" | "arm64" | "loongarch64" | "loongson3" | "mips64r6el" | "ppc64el" | "riscv64") {
                architectures.push(AOSCArchitecture {
                    name: arch.to_string(),
                    version: version.clone(),
                    size: pkg_size.to_string(),
                    download_url: format!("https://packages.aosc.io/files/{}/stable/{}/{}", arch, query, version),
                });
            }
        }
    }
    ...
    if architectures.is_empty() {
        let download_regex = Regex::new(r#"<a[^>]+href="/files/([a-z0-9]+)/[^"]*"[^>]*>\s*([0-9.]+\s*[KMGT]?iB)\s*</a>"#).unwrap();
    ...
    if architectures.is_empty() {
        let known_archs = ["amd64", "arm64", "loongarch64", "loongson3", "mips64r6el", "ppc64el", "riscv64"];
        let mut size_map = std::collections::HashMap::new();
        
        // Look for size patterns in the HTML text
        let size_context_regex = Regex::new(r#"(amd64|arm64|loongarch64|loongson3|mips64r6el|ppc64el|riscv64)[^0-9]*?(\d+\.\d+\s*[KMGT]?iB)"#).unwrap();
        ...
        for &arch in &known_archs {
            if html.contains(arch) {
                let size = size_map.get(arch).cloned().unwrap_or_else(|| "unknown".to_string());
                architectures.push(AOSCArchitecture {
                    name: arch.to_string(),
                    version: version.clone(),
                    size,
...
fn format_aosc_response(packages: &[AOSCPackage], query: &str) -> String {
    ...
    for (i, package) in packages.iter().enumerate().take(3) {
        ...
        if !package.architectures.is_empty() {
            let arch_names: Vec<String> = package.architectures.iter()
...
fn format_aosc_not_found(package_name: &str) -> String {
    format!(
        ...
        package_name, AOSC_SEARCH_URL, urlencoding::encode(package_name)
    )
}
