...
async fn query_aur_api(package_name: &str) -> Result<AurPackage> {
    ...
    let response = client.get(&url).send().await.context("Failed to send AUR API request")?;

    if !response.status().is_success() {
        return Err(anyhow::anyhow!("AUR API returned status: {}", response.status()));
    }
    ...
    debug!("AUR API response: {} results for {}", aur_response.resultcount, package_name);
...
fn format_aur_response(package: &AurPackage, _query: &str) -> String {
    ...
    response.push_str(
        &format!("% AUR Git Clone: https://aur.archlinux.org/{}.git\n", package.name)
    );
    response.push_str(
        &format!("% Install with: yay -S {} or paru -S {}\n", package.name, package.name)
    );
