...
async fn query_nixos_packages(package_name: &str) -> Result<NixOSSearchResponse> {
    ...
    let search_url = format!("{}?channel=23.11&query={}", 
                           NIXOS_SEARCH_API, urlencoding::encode(package_name));
...
fn format_nixos_response(packages: &[NixOSPackage], query: &str) -> String {
    ...
    for (i, package) in packages.iter().enumerate().take(3) {
        ...
        output.push_str(&format!("nixos-url: {}{}\n", 
                               NIXOS_SEARCH_URL, urlencoding::encode(&package.package_pname)));
    }
...
fn format_nixos_not_found(package_name: &str) -> String {
    format!(
        ...
        package_name, NIXOS_SEARCH_URL, urlencoding::encode(package_name)
    )
}
