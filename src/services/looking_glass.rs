...
fn format_bird_output(data: &LookingGlassData, resource: &str) -> Result<String> {
    ...
    let mut routes_by_prefix: std::collections::HashMap<String, Vec<&PeerData>> = std::collections::HashMap::new();
    ...
    for (prefix, peers) in routes_by_prefix {
        ...
        for peer in peers {
            ...
            if !peer.extended_community.is_empty() {
                output.push_str(&format!("    # Extended Communities: {}\n", peer.extended_community));
            }
            ...
            output.push_str(&format!("    bgp_path.len = {};\n", peer.as_path.split_whitespace().count()));
            ...
            if !peer.community.is_empty() {
                ...
                for community in communities {
                    if community.contains(':') {
                        output.push_str(&format!("    bgp_community.add(({},{}));\n", 
    ...
    output.push_str(&format!("# Summary: {} routes from {} RRC collectors\n", total_routes, total_rrcs));
