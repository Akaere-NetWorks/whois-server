...
use crate::config::{PEERINGDB_LMDB_PATH, PEERINGDB_CACHE_TTL};
...
pub async fn query_peeringdb_asn(asn: &str) -> Result<String> {
    ...
    let asn_num: u32 = asn_number.parse()
    ...
    if !response.status().is_success() {
        ...
        return Err(anyhow::anyhow!("PeeringDB API request failed: {} - {}", status, error_body));
    }
    ...
    if pdb_response.data.is_empty() {
        let no_data_response = format!("% No network information found for ASN {} in PeeringDB", asn_num);
    ...
    result.push_str(&format!("% PeeringDB Network Information for AS{}\n", asn_num));
...
pub async fn query_peeringdb_ix(ix_id: &str) -> Result<String> {
    ...
    if pdb_response.data.is_empty() {
        let no_data_response = format!("% No Internet Exchange information found for ID {} in PeeringDB", ix_num);
        // Cache negative response for shorter time (1 hour)
        let cache_entry = PeeringDBCacheEntry { 
            response: no_data_response.clone(), 
            cached_at: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs()
    ...
    let mut result = String::new();
    result.push_str(&format!("% PeeringDB Internet Exchange Information for ID {}\n", ix_num));
    result.push_str("% Source: https://www.peeringdb.com/\n\n");
...
fn format_network_info(network: &NetworkInfo) -> String {
    ...
    if let Some(ipv6) = network.info_ipv6 {
        info.push_str(&format!("IPv6 Support:       {}\n", if ipv6 { "Yes" } else { "No" }));
    }
    
    if let Some(unicast) = network.info_unicast {
        info.push_str(&format!("Unicast:            {}\n", if unicast { "Yes" } else { "No" }));
    }
    
    if let Some(multicast) = network.info_multicast {
        info.push_str(&format!("Multicast:          {}\n", if multicast { "Yes" } else { "No" }));
    }
    
    if let Some(never_route_servers) = network.info_never_via_route_servers {
        info.push_str(&format!("Route Server Policy: {}\n", 
            if never_route_servers { "Never via route servers" } else { "Route servers OK" }));
    }
    ...
    if let Some(netixlan_set) = &network.netixlan_set {
        if !netixlan_set.is_empty() {
            ...
            for (i, ix_connection) in netixlan_set.iter().enumerate() {
                info.push_str(&format!("IX #{}: {}\n", i + 1, 
                    ix_connection.name.as_ref().unwrap_or(&format!("IX ID {}", ix_connection.ix_id))));
                info.push_str(&format!("  IX ID:            {}\n", ix_connection.ix_id));
                info.push_str(&format!("  Speed:            {} Mbps\n", ix_connection.speed));
                ...
                info.push_str(&format!("  Route Server:     {}\n", 
                    if ix_connection.is_rs_peer { "Yes" } else { "No" }));
                info.push_str(&format!("  BFD Support:      {}\n", 
                    if ix_connection.bfd_support { "Yes" } else { "No" }));
                info.push_str(&format!("  Operational:      {}\n", 
                    if ix_connection.operational { "Yes" } else { "No" }));
...
pub async fn process_peeringdb_query(query: &str) -> Result<String> {
    ...
    if query.to_uppercase().starts_with("AS") {
        ...
        Err(anyhow::anyhow!("Invalid PeeringDB query format. Use 'AS12345' for ASN or '1234' for IX ID"))
    }
}
