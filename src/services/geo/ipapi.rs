...
pub async fn query_ipapi(client: &Client, ip: &str) -> Result<IpApiResponse> {
    ...
    let url = format!("http://ip-api.com/json/{}?fields=status,message,country,countryCode,region,regionName,city,zip,lat,lon,timezone,isp,org,as,mobile,proxy,hosting,query", ip);
...
/// Query IP-API for geo-location information (blocking version)
pub fn query_ipapi_blocking(client: &reqwest::blocking::Client, ip: &str) -> Result<IpApiResponse> {
    debug!("Querying IP-API (blocking) for: {}", ip);
