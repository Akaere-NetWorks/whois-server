...
pub async fn query_meituan(client: &Client, ip: &str) -> Result<MeituanCombinedResponse> {
    ...
    if !ip_response.status().is_success() {
        ...
        return Err(anyhow::anyhow!("Meituan IP API request failed with status: {}", ip_response.status()));
    }
    ...
    let ip_data = ip_api_response.data
        .ok_or_else(|| anyhow::anyhow!("Meituan IP API returned no data"))?;
    
    // Step 2: Get city details using coordinates
    let city_url = format!("https://apimobile.meituan.com/group/v1/city/latlng/{},{}?tag=0", ip_data.lat, ip_data.lng);
...
/// Query Meituan API for geo-location information (blocking version)
pub fn query_meituan_blocking(client: &reqwest::blocking::Client, ip: &str) -> Result<MeituanCombinedResponse> {
    debug!("Querying Meituan API (blocking) for: {}", ip);
        ...
        .send()
    {
        Ok(city_response) => {
            if city_response.status().is_success() {
                let city_body = city_response.text()?;
                debug!("Meituan City API response body: {}", city_body);
        ...
        Err(e) => {
            warn!("Failed to query Meituan City API: {}", e);
            None
        }
    };
