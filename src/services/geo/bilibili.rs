...
pub async fn query_bilibili(client: &Client, ip: &str) -> Result<BilibiliIpResponse> {
    ...
    if !response.status().is_success() {
        ...
        return Err(
            anyhow::anyhow!("BiliBili API request failed with status: {}", response.status())
        );
    }
    ...
    let api_response: BilibiliIpResponse = serde_json
        ::from_str(&body)
...
pub fn query_bilibili_blocking(
    client: &reqwest::blocking::Client,
    ip: &str
) -> Result<BilibiliIpResponse> {
