...
pub async fn query_bilibili(client: &Client, ip: &str) -> Result<BilibiliIpResponse> {
    ...
    if !response.status().is_success() {
        warn!("BiliBili API returned non-success status: {}", response.status());
        return Err(anyhow::anyhow!("BiliBili API request failed with status: {}", response.status()));
    }
    ...
    if api_response.code != 0 {
        ...
        return Err(anyhow::anyhow!("BiliBili API returned error: {} - {}", api_response.code, api_response.message));
    }
    
    Ok(api_response)
}

/// Query BiliBili API for geo-location information (blocking version)
pub fn query_bilibili_blocking(client: &reqwest::blocking::Client, ip: &str) -> Result<BilibiliIpResponse> {
    debug!("Querying BiliBili API (blocking) for: {}", ip);
