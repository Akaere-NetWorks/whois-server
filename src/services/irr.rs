...
async fn query_irr_explorer_api(client: &reqwest::Client, resource: &str) -> Result<Vec<IrrResponse>> {
    let url = format!("https://irrexplorer.nlnog.net/api/prefixes/prefix/{}", urlencoding::encode(resource));
...
fn query_irr_explorer_api_blocking(client: &reqwest::blocking::Client, resource: &str) -> Result<Vec<IrrResponse>> {
    let url = format!("https://irrexplorer.nlnog.net/api/prefixes/prefix/{}", urlencoding::encode(resource));
...
fn format_irr_response(resource: &str, responses: &[IrrResponse]) -> Result<String> {
    ...
    for (i, response) in responses.iter().enumerate() {
        ...
        if let Some(origins) = &response.bgp_origins {
            if !origins.is_empty() {
                formatted.push_str(&format!("% BGP Origins: {}\r\n", 
                    origins.iter().map(|o| format!("AS{}", o)).collect::<Vec<_>>().join(", ")));
            }
        }
