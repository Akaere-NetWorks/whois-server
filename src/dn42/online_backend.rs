...
impl DN42OnlineFetcher {
    ...
    pub fn new() -> Result<Self> {
        ...
        let storage = create_shared_storage(&cache_db_path)
            .map_err(|e| anyhow::anyhow!("Failed to create LMDB storage for online cache: {}", e))?;
    ...
    pub async fn fetch_file(&mut self, object_type: &str, file_name: &str) -> Result<Option<String>> {
            ...
            Ok(response) => {
                if response.status().is_success() {
                    match response.text().await {
                        Ok(content) => {
                            info!("DN42 Online: Successfully fetched {}/{}, size: {} bytes", object_type, file_name, content.len());
                            ...
                            tokio::task::spawn_blocking(move || {
                                ...
                                .map_err(|e| anyhow::anyhow!("Failed to cache content in LMDB: {}", e))?;
                            
                            Ok(Some(content))
                        },
                        Err(e) => {
                            warn!("DN42 Online: Failed to read response body for {}/{}: {}", object_type, file_name, e);
                            Ok(None)
                        }
                    }
                    ...
                    warn!("DN42 Online: HTTP error {} for {}/{}", response.status(), object_type, file_name);
                    Ok(None)
                }
            },
            Err(e) => {
    ...
    pub async fn find_ipv4_network(&mut self, object_type: &str, ip: std::net::Ipv4Addr, query_mask: u8) -> Result<Option<String>> {
        debug!("DN42 Online: Searching for IPv4 network in '{}' for IP {} with mask /{}", object_type, ip, query_mask);
    ...
    pub async fn find_ipv6_network(&mut self, object_type: &str, ip: std::net::Ipv6Addr, query_mask: u8) -> Result<Option<String>> {
        debug!("DN42 Online: Searching for IPv6 network in '{}' for IP {} with mask /{}", object_type, ip, query_mask);
