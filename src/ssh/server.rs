...
impl SshServer {
    ...
    pub fn new(config: SshServerConfig) -> Result<Self> {
        ...
        let history = Arc::new(
            SshConnectionHistory::new(&history_db_path)
                .with_context(|| format!("Failed to initialize SSH connection history at {:?}", history_db_path))?
        );
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        // Load host key
        let host_key = self.cert_manager.load_host_key().await
            ...
            match listener.accept().await {
                Ok((stream, client_addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = Self::handle_connection(stream, client_addr, history, host_key, config).await {
    ...
    async fn handle_connection(
        ...
        let _session = server::run_stream(config, stream, handler).await
