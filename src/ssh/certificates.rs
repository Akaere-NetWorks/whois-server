...
impl SshCertificateManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        if !self.host_key_path.exists() {
            ...
            self.generate_host_key().await.with_context(|| "Failed to generate SSH host key")?;
        } else {
